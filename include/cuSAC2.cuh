...
//
// constexpr int kHostU32Size = sizeof(u32);  ///< 4
// constexpr int kHostU32Bit = kHostU32Size * 8;
// constexpr int kHostU32Pos = 5;
// constexpr int kHostU32ModMask = 31;
//
// // 设备端常量声明
// __constant__ int kU32Size;
// __constant__ int kU32Bit;
// __constant__ int kU32Pos;
// __constant__ int kU32ModMask;
//
// // 主机端问题常量声明
// int hostBitDomIntSize = -1;   // 一个bitDom[x]的长度
// int hostBitDomsIntSize = -1;  // 整个bitDom的长度
// int hostBitSupIntSize = -1;   // 一个约束的bitsup的int长度
// // 主机端常量
// const int hostVsSize = 10;      // 变量个数
// const int hostCsSize = 20;      // 约束个数
// const int hostMaxDomSize = 30;  // 最大论域数
//
// // 设备端常量声明
// // 一个bitDom[x]的长度
...
// // 变量个数
// __constant__ int kDeviceVsSize;
// // 约束个数
// __constant__ int kDeviceCsSize;
// // 最大论域数
...
//
// // 初始化常量
// inline void initialGPUConst() {
//   cudaMemcpyToSymbol(kU32Size, &kHostU32Size, sizeof(int));
//   cudaMemcpyToSymbol(kU32Bit, &kHostU32Bit, sizeof(int));
//   cudaMemcpyToSymbol(kU32Pos, &kHostU32Pos, sizeof(int));
//   cudaMemcpyToSymbol(kU32ModMask, &kHostU32ModMask, sizeof(int));
//
//   // 将常量数组从主机复制到设备常量内存
//   cudaMemcpyToSymbol(kDeviceU32Mask1, kHostU32Mask1, sizeof(kHostU32Mask1));
//   cudaMemcpyToSymbol(kDeviceU32Mask0, kHostU32Mask0, sizeof(kHostU32Mask0));
// }
//
// // 初始化问题常量
// inline void initialProblemGPUConst() {
//   // 将常量值从主机复制到设备常量内存
//   cudaMemcpyToSymbol(kDeviceBitDomIntSize, &hostBitDomIntSize, sizeof(int));
//   cudaMemcpyToSymbol(kDeviceBitDomsIntSize, &hostBitDomsIntSize, sizeof(int));
//   cudaMemcpyToSymbol(kDeviceBitSupIntSize, &hostBitSupIntSize, sizeof(int));
//   cudaMemcpyToSymbol(kDeviceVsSize, &hostVsSize, sizeof(int));
//   cudaMemcpyToSymbol(kDeviceCsSize, &hostCsSize, sizeof(int));
//   cudaMemcpyToSymbol(kDeviceMaxDomSize, &hostMaxDomSize, sizeof(int));
// }
//
// // 宏和全局函数定义
// #ifndef MIN
...
// //__global__ void enforceACMain(u32* bitDom, u32* bitSup, u32* M_Con, u32*
// // M_ConEvt, u32* M_ConPre, u32* M_VarPre)
// //{
// //
// //}
//
...
//     // balres = number whose ith bit is one if the ith's thread pred is true
//     // masked up to the current index in warp 线程内局部变量pred =
//     // 1，与掩码按位与但过滤掉超过该线程id的记录，只保留变量前面的判定
//     int b = __ballot(pred) & t_m;
//     // popc count the number of bit one. simply count the number predicated true
//     // BEFORE MY INDEX 计算只计算当前线程索引对应的前N个的位数之和
//     // 即为线程束内排他扫描
//     int t_u = __popc(b);
//
...
//     // warpTotals长度为4
//     if (w_l == warpSize - 1) warpTotals[w_i] = t_u + pred;
//
//     __syncthreads();
//
//     // 线程束id为0，线程束内线程id，若blockDim.x = 128，则w_l < 128/32 = 4
//     // 线程块内第一个线程束的前（4）个线程束工作，w_l <
//     // 活动线程束数（4），即每个线程束被一个线程运行
...
//         // printf("indice %i
//         // t_m=%i,j=%i,b_j=%i,w_i_u=%i\n",w_l,t_m,j,b_j,w_i_u);
//       }
//       warpTotals[w_l] = w_i_u;
//     }
//     __syncthreads();
//
//     if (pred) ConEvt[t_u + warpTotals[w_i] + BOffset[blockIdx.x]] = scp[idx].z;
//   }
// }
//
...
//   // 以约束数量启动
//   // P1
//   GenConPre<<<MCC_BLOCK, num_threads>>>(M_VarPre, MCC_BlocksCount, scope,
//                                         CS_SIZE);
//   cudaDeviceSynchronize();
//   // P2
//   thrust::exclusive_scan(MCC_BCount.begin(), MCC_BCount.end(),
//                          MCC_BOffset.begin());
//   cudaDeviceSynchronize();
//   // P3
//   // 每个约束一个线程进行归约,共享内存大小 =
//   // 一个块内线程束的个数,用来装载线程束计算结果
//   CompactQ<<<MCC_BLOCK, num_threads, sizeof(int) * (num_threads / warpSize)>>>(
//       M_VarPre, M_Con, MCC_BlocksOffset, scope, CS_SIZE);
// }
//
// #define GetBitSupIndexByINTPrstn(cid, x_val, y_val) \
//   (cid * BITSUP_INTSIZE + x_val * BITDOM_INTSIZE + y_val)
//
// __inline__ __host__ int2 HostGetBitSupIndexByTuple(int cid, int2 t) {
//   return make_int2(
//       cid * hostBitSupIntSize + t.x * hostBitDomIntSize + (t.y >> kHostU32Pos),
//       cid * hostBitSupIntSize + t.y * hostBitDomIntSize + (t.x >> kHostU32Pos));
// }
//
// __inline__ __device__ int2 DeviceGetBitSupIndexByTuple(int cid, int2 t) {
//   return make_int2(cid * kDeviceBitSupIntSize + t.x * kDeviceBitDomIntSize +
//                        (t.y >> kU32Pos),
//                    cid * kDeviceBitSupIntSize + t.y * kDeviceBitDomIntSize +
//                        (t.x >> kU32Pos));
// }
//
// __inline__ __host__ int HostGetBitSupIndexById(int cid) {
//   return cid * hostBitSupIntSize;
// }
//
// __inline__ __device__ int DeviceGetBitSupIndexById(int cid) {
//   return cid * kDeviceBitSupIntSize;
// }
//
// void DelGPUModel();
//
...
thrust::device_vector<int3> M_Con;
thrust::device_vector<int3> M_ConEvt;
thrust::device_vector<int> M_VarPre;
...
// inline int intsizeof(const int x) { return (int)ceil((float)x / U32_BIT); }
inline int intsizeof(const int nbits) {
...
//     // masked up to the current index in warp 线程内局部变量pred =
//     // 1，与掩码按位与但过滤掉超过该线程id的记录，只保留变量前面的判定
//     int b = __ballot_sync(0xFFFFFFFF, pred) & t_m;
//     // popc count the number of bit one. simply count the number predicated
//     true
//     // BEFORE MY INDEX 计算只计算当前线程索引对应的前N个的位数之和
//     // 即为线程束内排他扫描
//     int t_u = __popc(b);
...
//         // 010 000
//         // 100 000
//         int b_j = __ballot_sync(0xFFFFFFFF, warpTotals[w_l] & pow2i(j));
//         w_i_u += (__popc(b_j & t_m)) << j;
//         // printf("indice %i
//         // t_m=%i,j=%i,b_j=%i,w_i_u=%i\n",w_l,t_m,j,b_j,w_i_u);
//       }
...
//     __syncthreads();
//
//     if (pred) ConEvt[t_u + warpTotals[w_i] + BOffset[blockIdx.x]] =
//     scp[idx].z;
//   }
// }
...
//   // 每个约束一个线程进行归约,共享内存大小 =
//   // 一个块内线程束的个数,用来装载线程束计算结果
//   CompactQ<<<MCC_BLOCK, num_threads, sizeof(int) * (num_threads /
//   warpSize)>>>(
//       M_VarPre, M_Con, MCC_BlocksOffset, scope, CS_SIZE);
// }
...
__inline__ __device__ __host__ int2 GetBitSupIndexByTuple(const int cid,
                                                          const int2 t) {
  return make_int2(
...
inline void BuildBitModel(const HModel& xm) {
#pragma region 计算常量
  // 变量个数
...
#pragma region 约束网络信息
  ...
  //   printf("i = %d , M_Con = %d, M_ConEvt = %d, M_ConPre = %d\n", i,
  //   M_Con[i],
  //          M_ConEvt[i], M_ConPre[i]);
  // }
  M_Con.resize(CS_SIZE);
  M_ConPre.reserve(CS_SIZE);
  M_ConEvt.resize(CS_SIZE);
  for (int i = 0; i < CS_SIZE; ++i) {
    printf("scope[%d] = {%d, %d}\n", scope[i].z, scope[i].x, scope[i].y);
  }
  ...
  for (int i = 0; i < CS_SIZE; ++i) {
    ...
    //        M_ConEvt[i], M_ConPre[i]);
    int3 value = M_Con[i];
    std::cout << "i = " << i << " M_Con = " << value.z
              << " M_Con =" << M_ConPre[i] << std::endl;
  }
...
#pragma region 拷贝bitDom
  ...
  thrust::host_vector<int> h_vec_copy = M_VarPre;
  for (int i = 0; i < VS_SIZE; ++i) {
    for (int j = 0; j < BITDOM_INTSIZE; ++j) {
      int idx = GetBitDomIndex(i, j);
      printf("var = %d, j = %d, idx = %d, bitDom = %x, pre= %x\n", i, j, idx,
             bitDom[idx], h_vec_copy[i]);
    }
  }
#pragma endregion 拷贝bitDom
...
//	一些GPU常量
//////////////////////////////////////////////////////////////////////////
extern __device__ __managed__ int NUM_BD_BLOCK;
extern __device__ __managed__ int NUM_CS_SIZE_BLOCKS;
// 一个变量论域的int长度
extern __device__ __managed__ int BITDOM_INTSIZE;
// 整个变量集合的论域的int长度
extern __device__ __managed__ int BITDOMS_INTSIZE;
// 一个约束的bitsup的int长度
extern __device__ __managed__ int BITSUP_INTSIZE;
// 整个约束集合的bitsup的int长度
extern __device__ __managed__ int BITSUPS_INTSIZE;
// 子问题论域总长度
extern __device__ __managed__ int BITSUBDOMS_INTSIZE;
// 变量个数
extern __device__ __managed__ int VS_SIZE;
// 约束个数
extern __device__ __managed__ int CS_SIZE;
// 约束最大元数，目前仅支持二元
extern __device__ __managed__ int MAX_ARITY;
// 主问题约束压缩BLOCK数
extern __device__ __managed__ int MCC_BLOCK;
//////////////////////////////////////////////////////////////////////////
//	一些GPU变量
//////////////////////////////////////////////////////////////////////////
extern __device__ __managed__ int M_Qsize;

//////////////////////////////////////////////////////////////////////////
//  GPU约束记录信息，不可更改
//////////////////////////////////////////////////////////////////////////
//	每个变量的int大小
extern __device__ __managed__ int* vars_size;
// 存储约束的scope，类型int3，scope.x: x.id; scope.y: y.id; scope.z: c.id
extern __device__ __managed__ int3* scope;
// 最大dom
extern __device__ __managed__ int MAX_DOM_SIZE;
// subCon长度
extern __device__ __managed__ int SUBCON_SIZE;

// extern  __device__ __managed__ int BITDOM_SIZE;
// extern  __device__ __managed__ int
//  主问题数据结构，使用UM
//  表示约束网络论域
extern __device__ __managed__ u32* bitDom;
// 表示约束，不可修改
extern __device__ __managed__ uint2* bitSup;
////类似队列，存储约束id
// extern  __device__ __managed__ int *mainCon;
////子问题数据结构
// 表示子问题的约束网络论域。
extern __device__ __managed__ u32* bitSubDom;
////类似队列，存储子问题约束id subCon.x: variable，subCon.y: value，subCon.z:
/// c.id
// extern  __device__ __managed__ ushort3* subCon;
//  标记主问题变量域是否删减，初始化全部为1
extern __device__ __managed__ int* M_VarPre;
// 标记主问题约束是否需检查，初始化全部为1
extern __device__ __managed__ int* M_ConPre;
// 主问题约束传播队列(压缩版)
extern __device__ __managed__ uint3* M_ConEvt;
// 主问题约束传播队列
extern __device__ __managed__ uint3* M_Con;
// extern thrust::device_vector<uint3> M_Con;
// extern thrust::device_vector<uint3> M_ConEvt;
// extern thrust::device_vector<int> M_ConPre;
// 标记子问题变量域是否删减，初始化全部为1
extern __device__ __managed__ int* S_VarPre;
// 记录子问题变量域
extern __device__ __managed__ uint3* S_Var;
// 标记子问题约束是否需检查，初始化全部为1
extern __device__ __managed__ int* S_ConPre;
// 子问题约束传播队列(压缩版)
extern __device__ __managed__ int3* S_ConEvt;
// 子问题约束传播队列
extern __device__ __managed__ int3* S_Con;
//
extern int* MCC_BlocksCount;
extern int* MCC_BlocksOffset;

extern thrust::device_vector<int> MCC_BCount;
extern thrust::device_vector<int> MCC_BOffset;
//
// extern  __device__ __managed__ ushort4* subVar;
////标记子问题发生改动的变量id，初始化全部为0
// extern  __device__ __managed__ unsigned short* subEvtVar;
////标记子问题发生改动的约束id，初始化全部为1
// extern  __device__ __managed__ int* subEvtCon;

extern const u32 U32_MASK1[32];

extern const u32 U32_MASK0[32];
