 ...
 public:
  ...
  bool HasSupport(cpim::Tabular* tab, int var_id, int value) {
    ...
    for (const auto& tuple : tab->tuples()) {
      ...
      for (size_t i = 0; i < tab->scope.size(); ++i) {
        ...
        if (!tab->scope[i]->have(other_value, 0)) {
          tuple_valid = false;
          break;
        }
      }
  ...
  std::vector<std::set<int>> ComputeGACDomains() {
    ...
    for (size_t var_id = 0; var_id < n_->vars.size(); ++var_id) {
      cpim::IntVar* var = n_->vars[var_id];
      for (int v = var->head(0); v != cpim::Limits::INDEX_OVERFLOW;
           v = var->next(v, 0)) {
...
void PrintDomain(cpim::IntVar* var, int level) {
  std::cout << "  V" << var->id() << ": {";
  bool first = true;
  for (int v = var->head(level); v != cpim::Limits::INDEX_OVERFLOW; v = var->next(v, level)) {
    if (!first) std::cout << ", ";
    std::cout << v;
    first = false;
  }
  std::cout << "} (size=" << var->size(level) << ")" << std::endl;
}
...
int main(int argc, char* argv[]) {
  ...
  for (auto* var : network.vars) {
    PrintDomain(var, 0);
    for (int v = var->head(0); v != cpim::Limits::INDEX_OVERFLOW; v = var->next(v, 0)) {
