...
ConsistencyState lMaxRPC::enforce(vector<IntVar*>& x_evt, const int p) {
  ...
  while (!q_var_.empty()) {
    ...
    for (auto i : neighborhood[j->id()]) {
      ...
      for (int a = i->head(); a != Limits::INDEX_OVERFLOW;
           i->next_value(a)) {
...
bool lMaxRPC::have_pc_support(IntVar& i, const int a, IntVar& j, const int p) {
  ...
  if (last_pc_iaj != Limits::INDEX_OVERFLOW && j.have(last_pc_iaj))
    return true;
...
bool lMaxRPC::have_pc_wit(IntVar& i, const int a, const IntVar& j, int b,
  ...
  if (last_ac[cva_ikia] != Limits::INDEX_OVERFLOW) {
    ...
    if ((bitSup_[cva_ikia][d] & bitSup_[cva_jkjb][d] & k.bitDom()[d]).any()) {
      return true;
    }
  }
  ...
  // const int bitdom_size = k.bitDom().size();
  for (int c = 0; c < k.num_bit_; ++c)
    if ((bitSup_[cva_ikia][c] & bitSup_[cva_jkjb][c] & k.bitDom()[c]).any()) {
      last_ac[cva_ikia] = c;
      last_ac[cva_jkjb] = c;
      return true;
    }
...
int lMaxRPC::next_support_bit(IntVar& i, const int a, IntVar& j, const int v,
  ...
  const uint64_t b =
      ((bitSup_[idx_cia][get<0>(index)] & j.bitDom()[get<0>(index)]) >>
       get<1>(index))
          .to_ullong();
  ...
  for (int u = get<0>(index) + 1; u < j.num_bit_; ++u) {
    const uint64_t mask = (bitSup_[idx_cia][u] & j.bitDom()[u]).to_ullong();
...
bool lMaxRPC::have_no_PC_support(IntVar* i, const int a, IntVar* j) {
  ...
  for (int b = j->head(); b != Limits::INDEX_OVERFLOW;
       j->next_value(b)) {
    int PCWitness = true;
...
bool lMaxRPC::have_PC_wit(IntVar* i, const int a, IntVar* j, const int b,
  ...
  if (last_ac[cva_ikia] != Limits::INDEX_OVERFLOW) {
    const auto d = last_ac[cva_ikia];
    if ((bitSup_[cva_ikia][d] & bitSup_[cva_jkjb][d] & k->bitDom()[d])
            .any()) {
      return true;
    }
    // const auto d = last_ac[cva_ikia];
  ...
  for (int c = 0; c < k->num_bit_; ++c)
    if ((bitSup_[cva_ikia][c] & bitSup_[cva_jkjb][c] & k->bitDom()[c])
            .any()) {
      last_ac[cva_ikia] = c;
      last_ac[cva_jkjb] = c;
      return true;
