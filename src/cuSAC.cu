...
// / bit支持， uint2 bitSup[c][a][idx].y =
//     bitSup[c, y, a, idx] / __device__ uint2 * **d_bitSup;
// / __host__ uint2*** h_bitSup;
// / __device__ u32** d_bitDom;
// / __host__ u32** h_bitDom;
// / __device__ u32** __constant__ int D_NUM_BD_BLOCK;
...
//     GPU约束记录信息，不可更改
//     /////////////////////////////////////////////////////////////////////////
//     / 每个变量的int大小 __device__ __managed__ int* vars_size;
...
// / __device__ __managed__ int / 主问题数据结构，使用UM /
//     表示约束网络论域 __device__ __managed__ u32* bitDom;
// // 表示约束，不可修改
// __device__ __managed__ uint2* bitSup;
// ////类似队列，存储约束id
// / __device__ __managed__ int* mainCon;
// /// 子问题数据结构
// / 表示子问题的约束网络论域。 __device__ __managed__ u32* bitSubDom;
// ////类似队列，存储子问题约束id subCon.x: variable，subCon.y: value，subCon.z:
// ///c.id
// / __device__ __managed__ ushort3* subCon;
// / 标记主问题变量域是否删减，初始化全部为1 __device__ __managed__ int* M_VarPre;
...
// }
// //************************************
// / Method : intsizeof / FullName : intsizeof / Access : public /
//                                                        Returns
//     : int /
//       Qualifier : 获取用bit表示的int长度 / Parameter : const int x
// /************************************
