...
def run_case(binary, graph, domain, seed, args):
    cmd = [
        str(binary),
        "--instance",
        f"synthetic:{graph}",
...
def main():
    ...
    for graph in [g.strip() for g in args.graphs.split(",") if g.strip()]:
        for domain in domains:
            for seed in range(args.seeds):
                rows.append(run_case(binary, graph, domain, seed, args))
    ...
    print(
        "graph domain constraints bitSupKiB queueP95 queueMax "
        "latencyM bankConflicts maxBankAccess crossRatio hubs bram18 uram288 unknownRate"
    )
    for graph, graph_rows in by_graph.items():
        ...
        for domain, domain_rows in sorted(by_domain.items()):
            avg = lambda key: sum(r[key] for r in domain_rows) / len(domain_rows)
            print(
                graph,
                domain,
                ...
                f"{max(r['queue_occupancy_max'] for r in domain_rows)}",
                f"{avg('support_latency_cycles') / 1_000_000.0:.2f}",
                f"{avg('support_bank_conflicts'):.1f}",
                f"{max(r['support_max_bank_accesses'] for r in domain_rows)}",
