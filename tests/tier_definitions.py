...
- TIER1: 标准回归（35个，<60s）- 合并前验证
- TIER2: 完整验证（75个，<300s）- 发版前测试
...
SAMPLE_ROOT = PROJECT_ROOT / "samples/bench"
...
TIER0: List[str] = [
    # 1. 极小实例（可穷举验证）
    _resolve_path("samples/bench/test.xml"),                           # 3 变量
    _resolve_path("samples/bench/queens-4_ext.xml"),                   # 4 变量，N-Queens
...
def get_tier3() -> List[str]:
    ...
    # 添加 samples/bench/ 目录（包括 rand-2-23）
